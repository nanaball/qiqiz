 ...
 * 구현이 쉽고 이해하지 쉬운 인터페이스를 제공해야함. 
 * 
 * 배열(Array)은 데이터를 저장하는 자료구조의 일종. 
 * 동일한 자료형의 데이터를 연속된 공간에 순차적으로 나열 한 것 
 * 배열은 메모리상에서 연속된 공간에 데이터를 저장하며 각 데이터는 인덱스(index)라는 숫자로 구분된다. 
...
public class UseArrayExample {
	...
	public static void main(String[] srgs) {
		...
		// 3. 배열 생성 시에 값이 정해져 있지 않을 때는 저장할 공간만 생성 가능하며 생성된 각 항목의 공간에는 기본값으로 자동 초기화된다.
