...
public class ExcutorsExample {
	...
	public static void main(String[] args) {
		...
		/*
		 * thread pool을 생성 하는 정적 메소드 newCachedThreadPool() 초기 스레드 수, 코어 스레드 수 0 - 최대 스레드
		 * 수 Integer.MAX_VALUE newFixedThreadPool(int value); 초기 스레드 수 : 0 - 코어 스레드 수 -
		 * 최대 스레드수 : value
		 * 
		 * 초기 스레드 수 : 스레드 풀이 생성 될 때 기본 적으로 생성 되는 스레드 수 코어 스레드 수 : 스레드가 증가 한 후 사용하지 않는
		 * 스레드를 제거하지 않고 최소한으로 유지할 개수 최대 스레드 수 : 스레드 풀에서 생성 관리하는 최대 스레드 수
		 */
		...
		for (int i = 0; i < 16; i++) {
			...
			/*
			 * try { System.out.println("스레드 기능 완료 시 까지 blocking"); future.get(); } catch
