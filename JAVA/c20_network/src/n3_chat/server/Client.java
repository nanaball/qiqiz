...
/**
 * Socket으로 연결된 client의 정보를 저장하고 개별 송/수신을 담당할 class
 */
public class Client {

	/**
	 * 연결된 client socket 정보를 저장할 filed OutputStream InputStream
	 */
	Socket socket;
	...
	/**
	 * socket으로 연결된 client에게 server에서 필요한 정보를 매개변수로 넘겨받아 출력
	 * 
	 * @param message
	 */
	...
	/**
	 * client에서 출력된 데이터를 수신 연결된 모든 client에게 메세지 출력
	 */
	public void receive() {
		...
		Thread t = new Thread(new Runnable() {
			@Override
			public void run() {
				// 반복하면서 client에서 출력된 데이터를 수신받아 모든 클라이언트에 출력
				while (true) {
					InputStream is = null;
